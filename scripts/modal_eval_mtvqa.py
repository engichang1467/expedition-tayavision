...
app = modal.App("tayavision-eval-mtvqa")

MODEL_NAME = "TrishanuDas/tayavision-instruct-665k"

SPLITS = ["ar", "de", "fr", "it", "ja", "kr", "ru", "th", "vi"]
...
def run_evaluation(
    ...
    task = f"mtvqa_{split}"
    out_dir = f"/results/mtvqa_{split}"
...
# Usage:
#   modal run scripts/modal_eval_mtvqa.py --split ar
#   modal run scripts/modal_eval_mtvqa.py --split all     (all 9 splits in parallel)
#   modal run scripts/modal_eval_mtvqa.py --split ar --limit 5  (test)
...
def main(
    ...
    print("\nEvaluation complete. Results stored in evaluation/results/mtvqa_{split}/")
